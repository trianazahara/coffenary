class Pesanan {
  constructor({ repo, snap, now = () => Date.now() }) {
    this.repo = repo;
    this.snap = snap;
    this.now = now;
  ...
  async checkout(req, res, next) {
    const { id_pengguna, id_cabang, tipe_pesanan, id_meja, items } = req.body || {};
    ...
    try {
      ...
      const id_pesanan = await this.repo.buatPesanan(conn, {
        nomor_pesanan: nomorPesanan, id_pengguna, id_cabang, tipe_pesanan, id_meja, total
      });
